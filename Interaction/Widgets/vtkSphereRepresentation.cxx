  ...
  if ( this->InteractionState == vtkSphereRepresentation::Translating )
    ...
    vtkAssemblyPath* path =
      this->Renderer->GetRenderWindow()->GetInteractor()->GetAssemblyPath(
        e[0], e[1], 0., this->SpherePicker,
        this->Renderer, this, this->ManagesPicking);
...
int vtkSphereRepresentation::ComputeInteractionState(int X, int Y, int vtkNotUsed(modify))
{
  ...
  if ( ! handlePicked )
    {
    path = this->Renderer->GetRenderWindow()->GetInteractor()->GetAssemblyPath(
      X, Y, 0., this->SpherePicker,
      this->Renderer, this, this->ManagesPicking);
